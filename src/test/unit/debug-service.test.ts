...
describe('DebugService', () => {
  ...
  describe('app state collection', () => {
    it('should collect comprehensive app state', async () => {
      ...
      vi.mocked(DatabaseService.getAssetsByBookmarkId).mockImplementation(async (bookmarkId: number) => {
        return mockAssets.filter(asset => asset.bookmark_id === bookmarkId) as any;
      });
      vi.mocked(DatabaseService.getSettings).mockResolvedValue(mockSettings);
